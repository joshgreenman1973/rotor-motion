...
- **Data:** real-time ADS-B from the [adsb.fi](https://adsb.fi) open data API
  (no key; personal, non-commercial use; 1 request/second), filtered to rotorcraft
  (ADS-B emitter category **A7**). Helicopters leave fading "radar" wakes colored by
  altitude; hover for type, operator, altitude, speed. airplanes.live, the original
  source, closed anonymous access in August 2026. adsb.fi sends no CORS header, so
  the live page cannot fetch it directly from the browser yet; the recorder, which
  runs server-side, is unaffected.
